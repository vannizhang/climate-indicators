...
type Props = {
    xScale: ScaleBand<String | number> | ScaleLinear<number, number> | ScaleTime<number, number>;
...
const PointerEventsOverlay:React.FC<Props> = ({
    ...
    const updateVerticalRefLinePos = (): void => {
        ...
        let xPos:number = itemOnHover.current 
            ? itemOnHover.current.xPosition 
            : 0;

        vRefLine.attr('x1', xPos)
            .attr('x2', xPos)
            .style('opacity', opacity);
    };

    const getDataByMousePos = (
        mousePosX: number
        ...
        const offset = ('bandwidth' in xScale) 
            ? xScale.bandwidth() / 2
            : 0;
        ...
        if(mousePosX < offset || mousePosX > width - offset){
            ...
            const index = mousePosX < offset 
                ? 0 :  
                xDomain.length -1;

            const value = xDomain[index];

            const xPosition = ('bandwidth' in xScale) 
                ? xScale(value) + offset 
                : xScale(+value);
        ...
        for (let i = 0, len = xDomain.length; i < len; i++) {
            ...
            if (mousePosX >= currItemPos && mousePosX <= nextItemPos) {
                ...
                itemIndex = distToCurrItem < distToNextItem 
                    ? i 
                    : nextItemIndex;

                xPosition = distToCurrItem < distToNextItem 
                    ? currItemPos
                    : nextItemPos;
    ...
    return (
        <g 
            className="pointer-event-overlay-group"
            ref={containerG}
        />
    )
}
